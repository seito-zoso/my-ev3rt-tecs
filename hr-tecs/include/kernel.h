 ...
 */
#ifndef PROBE_MEM_WRITE_SIZE
#define PROBE_MEM_WRITE_SIZE(p_var, size) \
				(cdmid == TDOM_KERNEL || prb_mem((void *)(p_var), size,	\
											TSK_SELF, TPM_WRITE) == E_OK)
#endif /* PROBE_MEM_WRITE_SIZE */

#ifndef PROBE_MEM_WRITE_COUNT
#define PROBE_MEM_WRITE_COUNT(p_var, type, count) \
				(ALIGN_TYPE(p_var, type) && (cdmid == TDOM_KERNEL		\
					|| prb_mem((void *)(p_var), sizeof(type) * (count),	\
											TSK_SELF, TPM_WRITE) == E_OK))
#endif /* PROBE_MEM_WRITE_COUNT */

#ifndef PROBE_MEM_WRITE
#define PROBE_MEM_WRITE(p_var, type) \
				PROBE_MEM_WRITE_COUNT(p_var, type, 1U)
#endif /* PROBE_MEM_WRITE */

#ifndef PROBE_MEM_READ_SIZE
#define PROBE_MEM_READ_SIZE(p_var, size) \
				(cdmid == TDOM_KERNEL || prb_mem((void *)(p_var), size,	\
											TSK_SELF, TPM_READ) == E_OK)
#endif /* PROBE_MEM_READ_SIZE */

#ifndef PROBE_MEM_READ_COUNT
#define PROBE_MEM_READ_COUNT(p_var, type, count) \
				(ALIGN_TYPE(p_var, type) && (cdmid == TDOM_KERNEL		\
					|| prb_mem((void *)(p_var), sizeof(type) * (count),	\
											TSK_SELF, TPM_READ) == E_OK))
#endif /* PROBE_MEM_READ_COUNT */

#ifndef PROBE_MEM_READ
#define PROBE_MEM_READ(p_var, type)	\
				PROBE_MEM_READ_COUNT(p_var, type, 1U)
#endif /* PROBE_MEM_READ */

#ifndef PROBE_MEM_SIZE
#define PROBE_MEM_SIZE(p_var, size) \
				(cdmid == TDOM_KERNEL || prb_mem((void *)(p_var), size,	\
									TSK_SELF, TPM_WRITE|TPM_READ) == E_OK)
#endif /* PROBE_MEM_SIZE */

#ifndef PROBE_MEM_COUNT
#define PROBE_MEM_COUNT(p_var, type, count) \
				(ALIGN_TYPE(p_var, type) && (cdmid == TDOM_KERNEL		\
					|| prb_mem((void *)(p_var), sizeof(type) * (count),	\
									TSK_SELF, TPM_WRITE|TPM_READ) == E_OK)
#endif /* PROBE_MEM_COUNT */

#ifndef PROBE_MEM
#define PROBE_MEM(p_var, type)	PROBE_MEM_COUNT(p_var, type, 1U)
#endif /* PROBE_MEM */
