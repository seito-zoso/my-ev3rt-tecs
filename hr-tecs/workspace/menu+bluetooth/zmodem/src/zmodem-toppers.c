...
/*
#define SYSLOG_ZRT( a , b ) syslog( (a) , (b) )
#define SYSLOG_ZRF( a , b ) syslog( (a) , (b) )
*/
#define SYSLOG_ZRT( a , b ) 
#define SYSLOG_ZRF( a , b ) 
...
void zmodem_recv_task(intptr_t unused) {
	...
	wait_file_sender();
	SYSLOG_ZRT(LOG_NOTICE, "zrT-1");
	i=zmodem_recv_init(&zm);
	SYSLOG_ZRT(LOG_NOTICE, "zrT-2");
...
//    FILE *fp;
    ...
    SYSLOG_ZRF(LOG_NOTICE, "zrf-1");

    recv_file = fopencookie(&mycookie,"w+", memfile_func);

    SYSLOG_ZRF(LOG_NOTICE, "zrf-2");
    /**
     * Act ZMODEM task
     */
...
//	SYSLOG_ZRF(LOG_NOTICE, "zrf-3");
    ...
    SYSLOG_ZRF(LOG_NOTICE, "zrf-10");
    fclose(recv_file);
    SYSLOG_ZRF(LOG_NOTICE, "zrf-11");
